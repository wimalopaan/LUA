...
local function createSlider(i)    
    return { type = "box", flexFlow = lvgl.FLOW_COLUMN, w = settings.sliders[i].width, children = {
            ...
            { type = "verticalSlider", min = -1024, max = 1024, h = 180,
                get = (function() return getShmVar(settings.sliders[i].shm) end),
                set = (function(v) setShmVar(settings.sliders[i].shm, v); end),
                color = settings.sliders[i].color
            },
            { type = "label", text = (function() return math.floor(getShmVar(i) / 10.24 + 0.5) .. "%"; end)}
            }
        };
end
...
local function createSetting(i)
    ...
    return { type = "box", flexFlow = lvgl.FLOW_ROW, children = {
            ...
            {type = "numberEdit", min = 1 , max = 16, w = 60, get = (function() return settings.sliders[i].shm; end), set = (function(v) settings.sliders[i].shm = v; end) },                             
        }
    }
end
